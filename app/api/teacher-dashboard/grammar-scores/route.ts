...
export async function POST(request: NextRequest) {
  try {
    ...
    if (filters) {
      ...
      if (filters.grammarTopic) {
        query = query.eq('grammar_topic', filters.grammarTopic);
      }
      ...
      // Course filter
      if (filters.course) {
        query = query.eq('course_name', filters.course);
      }
    }

    const { data: grammarScores, error } = await query;
    ...
    const uniqueUserIds = [...new Set(grammarScores?.map(score => score.user_id) || [])];
    ...
    // Enrich grammar scores with user details
    const enrichedScores = grammarScores?.map(score => ({
      ...score,
    ...
    const stats = {
      ...
      averageScore: enrichedScores?.length 
        ? enrichedScores.reduce((acc, s) => acc + (parseFloat(s.percentage_score) || 0), 0) / enrichedScores.length
        : 0,
      uniqueLearners: uniqueUserIds.length,
      scoreDistribution: calculateScoreDistribution(enrichedScores || []),
      grammarTopics: [...new Set(enrichedScores?.map(s => s.grammar_topic) || [])],
    ...
    console.error('Error in grammar-scores API:', error);
    return NextResponse.json(
      { error: 'Failed to fetch grammar scores data' },
      { status: 500 }
    );
  }
}

function calculateScoreDistribution(scores: Array<{ percentage_score: string }>) {
  const distribution = {
    '0-20': 0,
    '21-40': 0,
    '41-60': 0,
  ...
  scores.forEach(score => {
    const value = parseFloat(score.percentage_score) || 0;
